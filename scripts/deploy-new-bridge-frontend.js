...
module.exports = async function (exit) {
  if (process.argv.length < 7) {
    console.log(
      `Usage: ${process.argv.join(" ")} <default frontend> <bridge name>`
    );
    exit(1);
  }
  ...
  try {
    ...
    const registerBridgeTx = await controller.setBridgeFrontend(
      bridge.address,
      bridgeName
    );
    console.log(
      "Registering new frontend as frontend bridge. tx: ",
      registerBridgeTx
    );
    // #4 Register the controller in the new frontend.
    const registerControllerTx = await bridge.setController(controllerAddress);
    console.log(
      "Registering the controller in the bridge frontend. tx: ",
      registerControllerTx
    );
    exit(0);
  } catch (e) {
    exit(e);
