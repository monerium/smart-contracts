...
module.exports = async function(exit) {
  ...
  try {
    ...
    await NewControllerAndBridgeFrontend.link("SmartTokenLib", smartTokenLib.address);
    await NewControllerAndBridgeFrontend.link("TokenStorageLib", tokenStorageLib.address);
    ...
    await NewControllerAndBridgeFrontend.link("MintableTokenLib", mintableTokenLib.address);
    //Controller
    const instance = await NewControllerAndBridgeFrontend.new();
    console.log("Contract deployed to: ", instance.address, " tx: ", instance.transactionHash);
    exit(0);
  } catch (e) {
    exit(e);
  }
