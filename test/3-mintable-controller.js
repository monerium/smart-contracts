...
contract("MintableController", (accounts) => {
  ...
  it("should fail to mint 666 new tokens from non-system account", async () => {
    await truffleAssert.reverts(
      controller.mintTo_withCaller(accounts[0], accounts[0], 666, {
        from: accounts[0],
      })
    );
  });
  ...
  it("should fail to burnFrom 888 new tokens from non-system account", async () => {
    ...
    await controller.mintTo_withCaller(system, wallet.address, 888, {
      from: system,
    });
    await controller.removeSystemAccount(nonSystem);
    ...
    await truffleAssert.reverts(
      controller.burnFrom_withCaller(
        nonSystem,
        wallet.address,
        888,
        hash,
    ...
    assert.strictEqual(
      balance1.toNumber(),
      balance0.toNumber() + 888,
      "burnFrom should fail from non-system accont"
    );
    await controller.burnFrom_withCaller(
      system,
      wallet.address,
      888,
      hash,
    ...
    assert.strictEqual(
      balance2.toString(),
      balance0.toString(),
      "end state should be equal to initial state"
    );
  });
  ...
  it("should fail to burnFrom 999 new tokens using an incorrect signature", async () => {
    const wallet = wallets["trust wallet"];
    const balance0 = await controller.balanceOf(wallet.address);
    await controller.mintTo_withCaller(system, wallet.address, 999, {
      from: system,
    });
    ...
    assert.strictEqual(
      balance1.toNumber(),
      balance0.toNumber() + 999,
      "burnFrom should fail using a wrong signature"
    );
    await controller.burnFrom_withCaller(
      system,
      wallet.address,
  ...
  it("should mint 82300 tokens to a non-system address", async () => {
    await controller.mintTo_withCaller(system, address, 82300, {
      from: system,
    });
  ...
  for (var name in wallets) {
    ...
    it(`should burn 82000 tokens from a non-system address [${name}]`, async () => {
      await controller.mintTo_withCaller(system, wallet.address, 82000, {
        from: system,
      });
      const balance0 = await controller.balanceOf(wallet.address);
