...
contract("SmartController", (accounts) => {
  ...
  it("should transfer 3400 tokens to second account", async () => {
    await controller.transfer_withCaller(system, accounts[2], 3400, {
      from: system,
    });
    const balance = await controller.balanceOf(accounts[2]);
  ...
  it("should fail transferring 1840 tokens from a blacklisted account", async () => {
    ...
    await truffleAssert.reverts(
      controller.transfer_withCaller(accounts[2], accounts[1], 1840, {
        from: accounts[2],
      })
    );
  });
  ...
  it("should transfer 12 tokens to second account using transferFrom", async () => {
    await validator.unban(accounts[2]);
    await controller.approve_withCaller(accounts[2], accounts[1], 20, {
      from: accounts[2],
    });
  ...
  it("should fail transferring 22 tokens from a blacklisted account using transferFrom", async () => {
    ...
    controller.approve_withCaller(accounts[1], accounts[2], 30, {
      from: accounts[1],
    });
    await truffleAssert.reverts(
      controller.transferFrom_withCaller(
        accounts[2],
        accounts[1],
        accounts[0],
        22,
  ...
  it("should transfer 34 tokens using transferAndCall", async () => {
    ...
    const initial = await controller.balanceOf(accounts[0]);
  ...
  for (var name in wallets) {
    ...
    it(`should be able to recover the balance of a known address to a new address [${name}]`, async () => {
      await controller.transfer_withCaller(system, wallet.address, 13, {
        from: system,
      });
      ...
      try {
        await controller.recover_withCaller(
          system,
          wallet.address,
          account,
          hash,
  ...
  it("should fail to recover from a non-system account", async () => {
    ...
    await controller.transfer_withCaller(system, wallet.address, 15, {
      from: system,
    });
    const sig = wallet.signature.replace(/^0x/, "");
    const initial = await controller.balanceOf(accounts[0]);
    ...
    const balanceTo = await controller.balanceOf(accounts[0]);
    assert.equal(
      balanceTo.toNumber(),
      initial.toNumber(),
