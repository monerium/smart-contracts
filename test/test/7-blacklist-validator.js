...
contract("BlacklistValidator", (accounts) => {
  ...
  beforeEach("setup blacklist validator", async () => {
    ...
    await validator.addSystemAccount(system);
    await validator.addAdminAccount(admin);
  });
  ...
  it("shoud succeed in banning account from an admin account", async () => {
    await validator.ban(accounts[6], { from: admin });
    const success = await validator.blacklist(accounts[6]);
    assert.strictEqual(success, true, "unable to ban account");
  });

  it("should fail in unbanning account from an admin account", async () => {
    await truffleAssert.reverts(validator.unban(accounts[6], { from: admin }));
    await validator.unban(accounts[6], { from: owner });
  });

  it("should succeed in banning account from a system account", async () => {
    await validator.ban(accounts[6], { from: system });
    const success = await validator.blacklist(accounts[6]);
    assert.strictEqual(success, true, "unable to ban account");
  });

  it("should fail in unbanning account from an system account", async () => {
    await truffleAssert.reverts(validator.unban(accounts[6], { from: admin }));
    await validator.unban(accounts[6], { from: owner });
  });
